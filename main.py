...
def run(source, destination):
    ...
    inputs = {"A": airportA, "B": airportB, "C": airportC, "D": airportD, "E":airportE}
    # startVertex = inputs.get(input())
    # endVertex = inputs.get(input())
    startVertex = inputs.get(source)
    endVertex = inputs.get(destination)
    ...
    if (earliestArrivalTime != float("inf")):
        return f'The earliest arrival time for the airport {endVertex} from airport {startVertex} is {earliestArrivalTime}:00. Hours'
    else:
        return f'No flight from airport {startVertex} to airport {endVertex} after {startT}:00.'
